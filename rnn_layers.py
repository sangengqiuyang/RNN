...
class RNN(Layer):
    ...
    def forward(self, inputs):
        ...
        N, T, D = inputs.shape
        output = np.zeros((N, T, len(self.bias)))
        ...
        for i in range(T):
            x = inputs[:, i, :]
            output[:, i, :] = self.cell.forward([x,pre_h] ,U,W,b)
            pre_h = output[:, i, :]
    ...
    def backward(self, out_grad, inputs):
        ...
        for i in range(T - 1, -1, -1):
            if i > 0:
                 dx_t,dU_t, dW_t, db_t = self.cell.backward(out_grad[:, i, :] + dprev_h_t, [inputs[:, i, :], output[:, i-1, :]], U, W, b)
            else:
                dx_t,dU_t, dW_t, db_t = self.cell.backward(out_grad[:, i, :] + dprev_h_t, [inputs[:, i, :],  np.zeros_like(output[:, 0, :])], U, W, b)
            
            in_grad[:, i, :] = dx_t[0]

            dprev_h_t = dx_t[1]

            self.kernel_grad += dU_t

            self.r_kernel_grad += dW_t
...
class BidirectionalRNN(Layer):
    ...
    def backward(self, out_grad, inputs):
        ...
        forward_in_grad = self.forward_rnn.backward(out_grad[:, :, :int(H/2)], inputs)
 
        backward_in_grad = self.backward_rnn.backward(self._reverse_temporal_data(out_grad[:, :, int(H/2):], mask),self._reverse_temporal_data(inputs, mask))

        in_grad = (forward_in_grad + self._reverse_temporal_data(backward_in_grad, mask))
